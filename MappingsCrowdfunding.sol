...
// Second iteration of the Crowdfunding SC. Now every project is an element of the mapping.
...
contract CrowdFunding {
    ...
    modifier inTime(address _crowdFundingAddress) {
        ...
        require(
            block.timestamp <= projects[idx].deadline,
    ...
    modifier isAchieved(address _crowdFundingAddress) {
        uint256 idx = getIndexByAddress(_crowdFundingAddress);
        require(
            projects[idx].balance >= projects[idx].ethGoal,
    ...
    function getIndexByAddress(address _address)
        private
        view
        returns (uint256)
    {
    ...
    function deleteProject(address _address) private {
        ...
        for (uint256 i = idx; i < projects.length - 1; i++) {
            projects[i] = projects[i + 1];
        }
    ...
    function makeDonation(address _crowdFundingAddress)
        public
        payable
        isNotOwner(msg.sender)
        inTime(_crowdFundingAddress)
        isNotAchieved(_crowdFundingAddress)
    {
        uint256 idx = getIndexByAddress(_crowdFundingAddress);
    ...
    function claim(address _crowdFundingAddress)
        public
        notInTime(_crowdFundingAddress)
        isNotAchieved(_crowdFundingAddress)
    {
    ...
    function withdrawFunds()
        public
        isOwner(msg.sender)
        isAchieved(msg.sender)
        isNotClaimed(msg.sender)
    {
